    ...
    auto atFrequency{[&VERBOSE, &behavior, &od4]() -> bool
      {
        ...
        auto pedalPosition = behavior.getPedalPositionRequest(); 
        auto groundSteering = behavior.getGroundSteeringAngle();

        cluon::data::TimeStamp sampleTime = cluon::time::now();
        od4.send(pedalPosition, sampleTime, 0);
        od4.send(groundSteering, sampleTime, 1);

        if (VERBOSE) {
         std::cout << "The speed is " << behavior.getPedalPositionRequest().position() << std::endl;
         std::cout << "The  groundsteering is " << behavior.getGroundSteeringAngle().groundSteering() << std::endl;
        }
